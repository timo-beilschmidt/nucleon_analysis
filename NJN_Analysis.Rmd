...
read_dia <- function(key, nrDiagram=4, file_str, src_str_alt){
...
analyze <- function(corrKey = "N-N",pathToData="/hiskp4/petschlies/nucleon-ff/cA211a.30.32/J125-k0p2/",T=64, n_src = 16,n_conf = 1224){
  ...
  for (t in c(1:T/2)) {
    ...
    cor_conf[t,] <- (cor_mean_src[t,] + cor_mean_src[T-t,])/2
...
plotM <- function(c){
	...
	plot(x=df$t[1:(c$T/2-1)]+1 , y=(df$m[1:(c$T/2-1)]), main = "N-N Correlator effective mass",ylab = "m_eff",xlab = "t", xlim=c(1,30),ylim = c(.15, 1.5) ,col=colVec[1],  pch=1,cex=0.4, cex.main=1,lwd = 0.2, frame.plot=FALSE)
	arrows((df$t[1:(c$T/2-1)]+1), Re(df$m[1:(c$T/2-1)]-df$std[1:(c$T/2-1)]),(df$t[1:(c$T/2-1)]+1), Re(df$m[1:(c$T/2-1)]+df$std[1:(c$T/2-1)]), length=0.02,lwd=.2, angle=90, code=3)
	for (ttau in c(2:8)) {
	   
	  df <- m_boot(corr = c$cor_conf, R = 5000, tau = ttau, tsize = c$T/2)
	  points(x=df$t[1:(c$T/2-1)]+1, y=(df$m[1:(c$T/2-1)]), pch=ttau,cex=0.4, col=colVec[ttau], lwd = 0.3)
	  arrows((df$t[1:(c$T/2-1)]+1), Re(df$m[1:(c$T/2-1)]-df$std[1:(c$T/2-1)]),(df$t[1:(c$T/2-1)]+1), Re(df$m[1:(c$T/2-1)]+df$std[1:(c$T/2-1)]), length=0.02, angle=90, code=3, lwd = .2)
	}
	legend("topleft",c("tau = 1","tau = 2","tau = 3","tau = 4","tau = 5","tau = 6","tau = 7","tau = 8"),cex=.4,col=colVec,pch=c(1:8))
}
plotM(c2)
```
...
plot_Ratio <- function(c2, c3){
    ...
    plot(x=df$t[1:(c2$T/2-1)]+1 , y=Re(df$m[1:(c2$T/2-1)]), main = "N-J-N linear response of effective mass to external bilinear current",ylab = "g_00",xlab = "t", xlim=c(0,30),ylim = c(-25, 50) ,col=colVec[1],  pch=1,cex=0.4, cex.main=1,lwd = 0.2, frame.plot=FALSE)
    arrows((df$t[1:(c2$T/2-1)]+1), Re(df$m[1:(c2$T/2-1)]-df$std[1:(c2$T/2-1)]),(df$t[1:(c2$T/2-1)]+1), Re(df$m[1:(c2$T/2-1)]+df$std[1:(c2$T/2-1)]), length=0.02,lwd=.2, angle=90, code=3)
    print(sprintf("tau = %i", 1))
   print(Re(df$m))
    for (ttau in c(2:8)) {
                   
        df <- boot_Ratio(c2,c3, R = 5000, tau = ttau)
        points(x=df$t[1:(c2$T/2-1)]+1, y=Re(df$m[1:(c2$T/2-1)]), pch=ttau,cex=0.4, col=colVec[ttau], lwd = 0.3)
        arrows((df$t[1:(c2$T/2-1)]+1), Re(df$m[1:(c2$T/2-1)]-df$std[1:(c2$T/2-1)]),(df$t[1:(c2$T/2-1)]+1), Re(df$m[1:(c2$T/2-1)]+df$std[1:(c2$T/2-1)]), length=0.02, angle=90, code=3, lwd = .2)
