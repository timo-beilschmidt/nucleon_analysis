...
#  print(apply(Re(apply(cor_m, MARGIN=c(1,3), mean)), MARGIN=1, mean))
 ...
 # print(cor_final)
  ...
  cf <- cf_meta(.cf=cf_orig(cf=Re(t(cor_mean_src[1:33,]))), T=64, symmetrised=TRUE)
  cf <- bootstrap.cf(cf)
...
plotAndCalc <- function(gi, gf, mom_tag, test){
    ...
    plot.cf(c2, log="y")
    plot.cf(c3, main="3pt-function Correlator", log="y") #legend_title="N-J-N")

    plot.effectivemass(bootstrap.effectivemass(c2, type="log"))
    #m_data <- plotM(c2)
    #plotData <- plot_Ratio(c2,c3)
    #tau_plot(plotData)
    dev.off()
...
plotAndCalc(gi, gf, mom_tag, test)
