...
analyze <- function(corrKey = "N-N",T=64, n_src = 16,n_conf = 1224, path_letter = "b", gi= "Gi_Cg5", gf = "Gf_Cg5", step = 24, mom_tag = "px00py00pz00", conf_start=0){
  ...
  #cor_conf <- array(0, dim = c(T/2, ((n_conf-conf_start)/step+1)))
  #load gamma matrices
 ...
 print(sprintf("%s, T=%i, n_src=%i, n_conf=%i, Gi = %s, Gf = %s", corrKey, T, n_src, ((n_conf-conf_start)/step+1),gi, gf))
    ...
    for (src in c(1:n_src)) {
      ...
      if(strcmp(corrKey, "N-N")){
        ...
        nn <- n1 + n2
          ...
          nn <- read_dia("N-qbGq-N", nrDiagram =6 , file_str, src_str_alt, mom_tag=mom_tag)
...
#  print(apply(Re(apply(cor_m, MARGIN=c(1,3), mean)), MARGIN=1, mean))
  ...
  cor_std <- apply(cor_mean_src, MARGIN=1, FUN= function(x){
                                                    sqrt(var(Re(x)))/((n_conf-conf_start)/step+1)
                                                 })
  print("Symmetrized:")
  print(cor_final)
...
plotM <- function(c){
   ...
   #y <- Re(data$tau1$m[5:15])
   # fit <- summary(nls(y ~ a))
   #lines(x=df$t[5:15]+1, y = rep(fit$coefficients[1], 11))
...
plotAndCalc <- function(gi, gf, mom_tag, test){
    ...
    c2<- analyze(corrKey = "N-N", gi= gi, gf = gf, n_conf=max_conf, step = stepwidth, mom_tag=mom_tag, path_letter = "a", conf_start = min_conf)
    # cf_orig(cf=c2)
    # cf_meta(.cf=try_cf, Time = c2$T, symmetrised=TRUE)
