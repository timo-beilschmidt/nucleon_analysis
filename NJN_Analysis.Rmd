...
library(binhf)
library(boot)
source('/home/timo/repo/nucleon_analysis/get_sl.R')
source('/home/timo/repo/nucleon_analysis/R_gamma.R')
...
```{r, echo=FALSE}
analyze <- function(corrKey = "N-N",pathToData="/home/timo/repo/data/N-J-N/smearing/",T=64, n_src = 16,n_conf = 96){
  # Read in data
  # n_conf <- 1224
  # n_src <- 16
  # T <- 64
  ...
  p <- projector(1, g$`0`)
  # Source coord list
  source_list <- get_source_coords_table(f = paste(pathToData,"source_coords.cA211a.30.32.nsrc16.tab", sep = ""))
...
plotCor <- function(c){
	...
	plot(x=df$t, y=(df$C), main = "N-N Correlator",ylab = "Corr",xlab = "t", xlim=c(1,32),col="blue",  pch=1, cex.main=1.5, frame.plot=FALSE, log = "y")
...
# m_std <- apply(m_boot$t,2,sd, na.rm = TRUE)
...
plotM <- function(c){
	...
	for (ttau in c(2:8)) {
	  ...
	  points(x=df$t[1:63]+1, y=(df$m[1:63]), pch=ttau,cex=0.15, col=colVec[ttau], lwd = 0.2)
