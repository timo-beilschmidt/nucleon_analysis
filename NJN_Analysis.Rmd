...
```{r analyse, echo=FALSE, results='asis'}
##############################################################
...
plot.ratio <- function(c2, c3 ,t1, t2, add=FALSE, .col="black"){
  ...
  ratio$se <- std
  M <- try(invertCovMatrix(cf$t[,par.t1:t2], boot.samples=TRUE), silent=TRUE)
...
calc.cf <- function(gi, gf, mom_tag, test, letter){
    ...
    c3 <- analyse(corrKey = "N-J-N", gi= gi, gf = gf, n_conf=max_conf, step = stepwidth, mom_tag = mom_tag, path_letter = letter, conf_start = min_conf)
