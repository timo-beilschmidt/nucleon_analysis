...
plot.2combs <- function(gi1, gf1, gi2, gf2, mom.tag){
        ...
        df1 <- bootstrap.ratio(c2, c3, R=5000)
        std1 = apply(df1$t, c(2), sd ) 
        c2 <- bootstrap.cf(cf_2pt[[p_tot_tag]][[gi2]][[gf2]])
        c3 <- bootstrap.cf(cf_3pt[[p_tot_tag]][[gi2]][[gf2]])
        df2 <- bootstrap.ratio(c2, c3, R=5000)
        std2 = apply(df2$t, c(2), sd ) 
        plotwitherror(x=c(1:(c2$T)) , y=Re(df1$t0[1:(c2$T)]), dy=Re(std1[1:(c2$T)]) , main = "N-J-N linear response of effective mass to external bilinear current",ylab = "g_00",xlab = "t", xlim=c(0,30),ylim = c(-30,30), col=colVec[1],  pch=1,cex=0.8, cex.main=1,lwd = 0.3, frame.plot=FALSE) 
        ratio <- cf_meta(.cf=cf_orig(cf=df1$t), T=c2$T, symmetrised=TRUE)
        ratio <- bootstrap.cf(ratio)
        ratio$se <- std1
        M <- diag(1/ratio$se[par.t1:par.t2]^2)
        ratio$ratiofit <- fit.constant(M=M, y = ratio$cf[par.t1:par.t2])
        lines(x=c(par.t1,par.t2),
                       y=c(ratio$ratiofit$par[1],ratio$ratiofit$par[1]),
                             col="black",
                             lwd=1.3)
        pcol <- col2rgb("black",alpha=TRUE)/255
        pcol[4] <- 0.65
              ...
              rect(xleft=ratio$ratiofit$t1, ybottom=ratio$ratiofit$t0[1]-ratio$ratiofit$se[1],
                              xright=ratio$ratiofit$t2, ytop=ratio$ratiofit$t0[1]+ratio$ratiofit$se[1],
                                         col=pcol,
                                         border=NA)
        pointswitherror(x=c(1:(c2$T)) , y=Re(df2$t0[1:(c2$T)]), dy=Re(std2[1:(c2$T)]) , main = "N-J-N linear response of effective mass to external bilinear current",ylab = "g_00",xlab = "t", xlim=c(0,30),ylim = c(-30,30), col=colVec[1],  pch=1,cex=0.8, cex.main=1,lwd = 0.3, frame.plot=FALSE) 
        ratio <- cf_meta(.cf=cf_orig(cf=df2$t), T=c2$T, symmetrised=TRUE)
        ratio <- bootstrap.cf(ratio)
        ratio$se <- std2
        M <- try(invertCovMatrix(cf$t[,par.t1:par.t2], boot.samples=TRUE), silent=TRUE)
        ...
        pcol[4] <- 0.65
              ...
              rect(xleft=ratio$ratiofit$t1, ybottom=ratio$ratiofit$t0[1]-ratio$ratiofit$se[1],
                              xright=ratio$ratiofit$t2, ytop=ratio$ratiofit$t0[1]+ratio$ratiofit$se[1],
                                         col=pcol,
                                         border=NA)
              return(invisible(list("ratio1"=df1, "ratio2"=df2)))
}
