...
analyze <- function(corrKey = "N-N",pathToData="/hiskp4/petschlies/nucleon-ff/cA211a.30.32/J125-k0p2/",T=64, n_src = 16,n_conf = 1224, gi= "Gi_Cg5", gf = "Gf_Cg5"){
  # Read in data
  # n_conf <- 1224
  # n_src <- 16
  # T <- 64
  ...
  cor_m <- array(0, dim = c(T, n_src, (n_conf/24+1)))
 ...
 print(sprintf("%s, T=%i, n_src=%i, n_conf=%i, Gi = %s, Gf = %s", corrKey, T, n_src, (n_conf/24+1),gi, gf))
  for (conf in seq(0, n_conf, 24)){
    for (src in c(1:n_src)) {
      ...
      for (t in c(1:T)) {
        ...
        cor[t,src,(conf/24+1)] <- trace(proj_p %*% nn[,,t])
        cor_m[t,src,(conf/24+1)] <- trace(proj_m %*% nn[,,t])
      } # end time loop
      
      # shift source location to 0
      cor[,src,(conf/24+1)] <- shift(cor[,src,(conf/24+1)], t_src, dir = "left")
      cor_m[,src,(conf/24+1)] <- shift(cor_m[,src,(conf/24+1)], t_src, dir = "left")
    } # end source loop
...
c2<- analyze(corrKey = "N-N", gi= gi, gf = gf)
```
...
```{r 3pt, echo = FALSE}
c3 <- analyze(corrKey = "N-J-N", gi= gi, gf = gf)
